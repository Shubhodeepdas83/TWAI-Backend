...
def extract_used_citations(response, citation_map, all_retrieved_documents):
    ...
    for num in sorted(used_citation_numbers):
        try:
            print(f"\nProcessing citation number: {num}")
            source_desc = citation_map.get(num)
            if not source_desc:
            ...
            if ".pdf" in source_desc.lower() and "http" not in source_desc.lower():
                print("Handling PDF citation...")
                pdf_found = False
                for doc in all_retrieved_documents:
                    metadata = doc.get("metadata", {})
                    pdf_path = metadata.get("PDF Path")
                    page_number = metadata.get("Page Number")
                    
                    print(f"Checking document: {pdf_path}, Page: {page_number}")
                    if not pdf_path or not page_number:
                        print("Missing PDF Path or Page Number, skipping...")
                        continue
                    ...
                    pdf_identifier = pdf_path.split('/')[-1] if pdf_path.startswith('http') else pdf_path
                    
                    if pdf_path in source_desc or pdf_identifier in source_desc:
                        print(f"Matching PDF found: {pdf_path}, Page: {page_number}")
                        processed_pages.add(page_id)
                        text_chunk = doc.get("page_content", "")
                        
                        print("Extracting image...")
                        img_path = extract_page_image_with_highlight(pdf_path, page_number, text_chunk) or extract_page_image(pdf_path, page_number)
                        base64_image = encode_image_to_base64(img_path) if img_path else None
                        
                        if base64_image:
                            print(f"Image successfully extracted for citation {num}")
                        else:
                            print(f"Failed to extract image for citation {num}")

                        used_citations[num] = {
                            "description": source_desc,
                            "isimg": bool(base64_image),
                ...
                if not pdf_found:
                    print(f"No matching PDF found for citation {num}, adding as text-only.")
                    used_citations[num] = {"description": source_desc, "isimg": False, "image_data": None}
            else:
                print(f"Citation {num} is not a PDF, adding as text-only.")
                used_citations[num] = {"description": source_desc, "isimg": False, "image_data": None}
        
        except Exception as e:
            print(f"Error processing citation {num}: {e}")
            used_citations[num] = {"description": "Error processing citation", "isimg": False, "image_data": None}
    
    print("\nFinal extracted citations:", used_citations)
